...
1. Run the fact sheet **alone** first — it writes the freeze. All three scripts live in the **config home**, which is `$env:CLAUDE_CONFIG_DIR` when it is set, else `$env:USERPROFILE\.claude` (`$HOME/.claude` off Windows) — so a second-subscription session runs its own deployed build rather than the primary home's. Substitute that directory for `<config home>` in the commands below. (Node accepts the forward slashes on Windows too. On Windows, run these through the PowerShell tool.)

       node "<config home>/handover-facts.mjs"
...
## Subagent prompt
    ...
    You are the /handover-check explainer. Below is a deterministic FACT SHEET — every number is already formatted (backticks on) and every classification/verdict is already resolved. Your ONLY job is to compose the report as fluent, insightful Style-B markdown prose. Do NOT recompute, reclassify, reformat, or add/remove backticks; do NOT invent numbers. If a value reads `?`, omit that detail. Every figure the report states comes from the fact sheet already backticked, so a number you typed yourself is a number you invented — if the sheet did not give it, do not write it. Never put "under", "below" or "less than" in front of a `<$…` value — the `<` already says it; write either the word or the glyph, never both.
    ...
    • **Warm rewrites** — if WARM_REWRITE_TAX is "(none)", skip this line ENTIRELY. Otherwise one prose line conveying it at its stated volume, gloss included.
